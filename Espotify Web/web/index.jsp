...
<body>
    ...
    <div class="main-content container">
        ...
        <div id="dynamicContent">
            <iframe src="testpage.jsp" width="100%" height="500px" frameborder="0"></iframe>
        </div>
    </div>
  ...
  btnGeneros.addEventListener('click', () => {
    ...
    // Hacer una solicitud AJAX al servlet para obtener los géneros
    fetch('SvObtenerGeneros')
        ...
        .then(data => {
            ...
            data.forEach(genre => {
                ...
                genreItem.textContent = genre; // Asumiendo que 'genre' es el nombre del género

                genreItem.addEventListener('click', () => {
                    console.log('Género seleccionado:', genre);
                    const variableurl = encodeURIComponent(genre);
                    console.log('Valor de variableurl:', variableurl);
                    
                    // Hacer la solicitud AJAX al servlet para obtener los álbumes de este género
                     const url = "SvObtenerAlbumxGenero?genero=" + variableurl;
                    console.log('URL de la solicitud:', url); // Verifica la URL generada
                    ...
                    fetch(url)
                        ...
                        .then(albumes => {
                            // Limpiar la lista de géneros y mostrar los álbumes
                            dynamicContent.innerHTML = '<div class="album-list"></div>';
                            const albumList = document.querySelector('.album-list');

                            albumes.forEach(album => {
                                const albumItem = document.createElement('div');
                                albumItem.className = 'album-item';
                                albumItem.textContent = album.nombre; // Asegúrate de acceder a la propiedad correcta
                                

                                // albumItem.innerHTML = `<strong>${album.nombre}</strong> (${album.anioCreacion})`; // Ejemplo
