...
<html>
    ...
    <head>
        ...
        <link rel="stylesheet" href="css/ConsultaListaRepMobile.css">
        <script>
...
<%
   ...
   <li onclick="seleccionarLista('<%= lista.getNombreListaRep().replaceAll(" ", "_") %>', '<%= lista.getNombreCliente().replaceAll(" ", "_") %>')">
    <div style="display: flex; align-items: center;">
        ...
        <div class="col-md-4">
            ...
            <form id="playlistForm_<%= lista.getNombreListaRep().replaceAll(" ", "_") %>_<%= lista.getNombreCliente().replaceAll(" ", "_") %>" 
                  action="SvSeleccionarLista" method="GET" class="playlist-form" style="display: inline;">
                <input type="hidden" name="nombreLista" value="<%= lista.getNombreListaRep() %>">
                <input type="hidden" name="nombreCliente" value="<%= lista.getNombreCliente() %>">
            </form>
            
            <!-- Botón de submit eliminado para usar el clic en el <li> -->
            <span class="playlist-title"><%= lista.getNombreListaRep() %> (por <%= lista.getNombreCliente() %>)</span>
        </div>
    </div>
</li>
                <%
  ...
  <div id="container mt-4" style="position: relative;"> <!-- Añadido position: relative para el contenedor -->
    <% 
...
function seleccionarLista(nombreLista, nombreCliente) {
    // Remplaza espacios por guiones bajos en el id del formulario
    const formId = "playlistForm_" + nombreLista.replace(/ /g, "_") + "_" + nombreCliente.replace(/ /g, "_");
